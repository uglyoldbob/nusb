...
jni::bind_java_type! {
    ...
    methods {
        ...
        /// whether or not the given object's class overrides hashCode(). As much as is reasonably practical,
        /// the default `hashCode` method defined by class `Object` does return distinct integers for distinct objects.
        static fn identity_hash_code(obj: JObject) -> jint,
    }
}
...
impl PermissionRequestTracker {
    ...
    fn on_activity_lifecycle_callback<'a>(
        ...
        let activity_ident_hash = JavaSystem::identity_hash_code(env, &activity)?;
        let class_name = env.get_object_class(activity)?.get_name(env)?.to_string();
        debug!("{callback_name} of {class_name}({activity_ident_hash}) received");
        match callback_name.trim() {
            "onActivityResumed" => {
                ...
                resume_event_mark.lock().unwrap().replace(ResumeEventMark {
                    resumed_activity_hash: activity_ident_hash,
                });
                ...
                std::thread::spawn(move || {
                    ...
                    if let Some(mark) = mark_lock.as_ref() {
                        if mark.resumed_activity_hash == activity_ident_hash {
                            drop(mark_lock);
                            if let Some(inner) = inner_weak.upgrade() {
            ...
            "onActivityPaused" | "onActivityStopped" => {
                ...
                if let Some(prev_mark) = mark_lock.as_ref() {
                    if prev_mark.resumed_activity_hash == activity_ident_hash {
